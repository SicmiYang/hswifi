...
public class HttpGetProxy{
	...
	public HttpGetProxy(int localport) {
		try {
			localPort = localport;
			localHost = EngineOptions.LOCAL_IP_ADDRESS;
	...
	public String prebuffer(String urlString,int size) throws Exception{
		...
		String filePath= SystemManager.getBufferDir()+"/"+fileName;
	...
	public String[] getLocalURL(String urlString) {
		...
		if (originalURI.getPort() != -1) {// URL带Port
			...
			serverAddress = new InetSocketAddress(remoteHost, EngineOptions.HTTP_PORT);// 使用80端口
			remotePort = -1;
