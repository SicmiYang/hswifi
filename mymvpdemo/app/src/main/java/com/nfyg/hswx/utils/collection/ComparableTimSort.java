...
class ComparableTimSort {
   ...
   ********************************************************/
	...
	public void doSort (Object[] a, int lo, int hi) {
		...
		//如果数组长度小于MIN_MERGE， 采用"mini-TimSort"，不合并
	...
	private static int countRunAndMakeAscending (Object[] a, int lo, int hi) {
		...
		//如果是降序，反转
		if (((Comparable)a[runHi++]).compareTo(a[lo]) < 0) { // 比较第一个索引位置和第二个索引位置下的元素的大小  ，该情况为降序
			while (runHi < hi && ((Comparable)a[runHi]).compareTo(a[runHi - 1]) < 0)
				runHi++;
			reverseRange(a, lo, runHi); //反转
		} else { // 该情况为升序
			while (runHi < hi && ((Comparable)a[runHi]).compareTo(a[runHi - 1]) >= 0)
				runHi++;
		}
	...
	/**
	 * 检查索引fromIndex，toIndex的范围，并抛出对应的异常 
	 * @param arrayLen 数组的长度
	 * @param fromIndex 起始索引 如果大于 toIndex 抛出 IllegalArgumentException 如果小于0 抛出 ArrayIndexOutOfBoundsException
	 * @param toIndex 结束索引  如果大于数组长度 抛出 ArrayIndexOutOfBoundsException
	 */
