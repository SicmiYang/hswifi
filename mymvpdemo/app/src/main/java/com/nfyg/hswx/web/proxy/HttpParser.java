package com.nfyg.hswx.web.proxy;
...
public class HttpParser {
	...
	public byte[] getRequestBody(byte[] source,int length){
		List<byte[]> httpRequest=getHttpBody(EngineOptions.HTTP_REQUEST_BEGIN,
				EngineOptions.HTTP_BODY_END,
				source,
				length);
	...
	public ProxyRequest getProxyRequest(byte[] requestBuffer){
		...
		if(result._body.contains(RANGE_PARAMS)==false)
			result._body = result._body.replace(EngineOptions.HTTP_BODY_END,
					"\r\n"+RANGE_PARAMS_0+EngineOptions.HTTP_BODY_END);
		Log.e(TAG, result._body);
		
		// 获取文件名
		String fileName=getURIPath(result._body);
		fileName= ProxyUtils.urlToFileName(fileName);
		result._prebufferFilePath= SystemManager.getBufferDir()+"/"+fileName;
	...
	public List<byte[]> getResponseBody(byte[] source,int length){
		List<byte[]> httpResponse=getHttpBody(EngineOptions.HTTP_RESPONSE_BEGIN,
				EngineOptions.HTTP_BODY_END,
				source,
				length);
	...
	private String getURIPath(String requestStr) {
		try {
			int startIndex = requestStr.indexOf(EngineOptions.HTTP_REQUEST_BEGIN)
					+ EngineOptions.HTTP_REQUEST_BEGIN.length();
			int endIndex = requestStr.indexOf(EngineOptions.HTTP_REQUEST_LINE1_END);
